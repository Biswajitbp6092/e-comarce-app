    ...
    <div className="productsItems shadow-lg rounded-md overflow-hidden">
      <div className="group imgWrapper w-[100%] overflow-hidden rounded-md relative">
        ...
        <div className="actions absolute top-[-15px] right-[5px] z-5 flex items-center gap-2 flex-col w-[50px] transition-all duration-300 group-hover:top-[15px] opacity-0 group-hover:opacity-100">
          <Button onClick={()=>context.handleOpenProductDetailsModal(true, props?.item)} className="!w-[35px] !h-[35px] !min-w-[35px] !rounded-full !text-black !bg-white hover:!bg-[#ff5252] hover:!text-white transition-all duration-300 ease-in">
      ...
      <div className="info p-3 py-5">
        ...
        <h3 className="text-[14px] title mt-1 font-[400] mb-1 text-[rgba(30,30,30,0.9)]">
          <Link to={`/product/${props?.item?._id}`} className="link transition-all block truncate max-w-[200px]">
            {props?.item?.name}
          </Link>
        </h3>
